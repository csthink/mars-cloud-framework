...
各服务实际占用的子区间登记在服务仓的 `docs/services.md`，避免两个服务挑中同一段。
可运行的完整接线示例见服务仓的 `mars-cloud-sample-service`。
...
**校验对象是「注册进来的业务码」**：只有通过 `ErrorCodeRegistrar` 注册的错误码才做归属、
越界与重复校验。框架自身兜底用的码（如参数校验失败的 `400`、未预期异常的 `500`）
**不经这套校验**，也不要求落在任何声明区段内——所以你在响应里看到 `code: "400"`
是正常的，不必去 `ranges` 里为它开一段。
...
文案查找有**四级**兜底，逐级下落直到命中：

| 级 | 来源 | key / 位置 |
| --- | --- | --- |
| ① | i18n 资源，规范 key | `error.code.<数字>` |
| ② | i18n 资源，纯数字 key | `<数字>` |
| ③ | 服务本地的兜底配置 | `config/exception-code.properties` 里的 `mars.codes[<数字>]` |
| ④ | 最终回落 | 数字码本身 |

第 ② 级是为兼容历史资源写法保留的，**容易被漏掉**——只按①③④实现会丢掉一档兜底。

另注意：`HttpException#getErrorMsg()` 只走 ① 与 ②，**不查 ③**；
只有响应体（advice 层）会走完四级。也就是说「要不要用兜底配置，取决于读的是异常自带文案还是响应体」。
